  ...
  if (loading) {
    return (
       <div className="flex flex-col items-center justify-center h-full">
         ...
         <p className="text-xl font-bold text-peppa-pink">Creating Lesson...</p>
         <p className="text-sm text-peppa-blue">Finding advanced vocabulary...</p>
       </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center h-full w-full max-w-4xl mx-auto">
      <div className="w-full flex justify-between items-center mb-4 px-2">
        <h2 className="text-xl md:text-2xl font-black text-peppa-blue">Minna Match</h2>
        <div className="text-lg md:text-xl font-bold bg-white px-4 py-1 md:py-2 rounded-full border-2 border-peppa-pink shadow-sm">
          {matchesCount} / 7
        </div>
        <button onClick={() => setIsPlaying(false)} className="bg-gray-100 p-2 rounded-full hover:bg-gray-200">
           ❌
        </button>
      </div>

      <div className="flex w-full gap-2 md:gap-12 flex-1 p-1 md:p-2 overflow-y-auto">
        {/* Left Column */}
        <div className="flex-1 space-y-2 md:space-y-3 pb-20">
          {leftCol.map(item => (
      ...
      {allMatched && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
          <div className="bg-white p-8 md:p-12 rounded-[3rem] text-center animate-bounce-in border-8 border-peppa-pink shadow-2xl w-full max-w-md">
             <div className="text-6xl md:text-8xl mb-6">🎉</div>
             <h3 className="text-3xl md:text-4xl font-black text-peppa-darkPink mb-4">Awesome!</h3>
             <p className="text-base md:text-lg text-gray-500 font-bold mb-8">You mastered 7 pairs!</p>
             <button 
               onClick={() => setIsPlaying(false)} 
               className="bg-peppa-green text-white text-lg md:text-xl font-black py-3 md:py-4 px-8 md:px-12 rounded-full shadow-lg border-b-8 border-peppa-grass hover:scale-105 active:scale-95 active:border-b-0 active:translate-y-2 transition-all w-full"
             >
               Play Again
             </button>
          </div>
...
const Card: React.FC<{ item: CardItem, onClick: () => void }> = ({ item, onClick }) => {
  let baseStyle = "w-full min-h-[4rem] md:min-h-[4.5rem] p-1 md:p-2 rounded-xl md:rounded-2xl border-b-4 font-bold text-sm md:text-lg transition-all duration-200 flex items-center justify-center cursor-pointer shadow-sm relative overflow-hidden group select-none touch-manipulation";
  let statusStyle = "";
  ...
  switch (item.status) {
    ...
    case 'error':
      ...
      break;
    default:
      statusStyle = "bg-white border-gray-200 text-slate-700 hover:bg-gray-50 hover:border-peppa-sky active:bg-gray-100";
  }
    ...
    <button onClick={onClick} className={`${baseStyle} ${statusStyle}`}>
      {item.type === 'jp' ? (
        <div className="flex items-center gap-1 md:gap-2">
            <Furigana text={item.text} size="sm" className="md:text-xl" />
            {/* Small icon to indicate audio is available */}
            {item.status === 'default' && (
              <Volume2 size={16} className="text-peppa-blue opacity-50 group-hover:opacity-100 hidden md:block" />
            )}
        </div>
      ) : (
